...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Dict[str, torch.Tensor]:
        ...
        for freq, seq_len, idx in zip(self.frequencies, self.seq_len, indices):
            ...
            sample[f'x_t{freq_suffix}'] = self.x_t[basin][freq][idx - seq_len + 1:idx + 1]#######################
    ...
    def _create_lookup_table(self, xr: xarray.Dataset):
        ...
        for basin in tqdm(basin_coordinates, file=sys.stdout, disable=self._disable_pbar):
            ...
            for freq in self.frequencies:
                ...
                the_date = df_resampled.index.date###################################################
                resutls = []###################################################
                for i in the_date:###################################################
                    resutls.append(10000*i.year + 100*i.month + i.day)###################################################
                x_t[freq] = np.array(resutls)###################################################
            ...
            with warnings.catch_warnings():
                ...
                flag = validate_samples(x_d=[x_d[freq] for freq in self.frequencies] if self.is_train else None,
                                        x_t=[x_t[freq] for freq in self.frequencies] if self.is_train else None,###############################
            ...
            if valid_samples.size > 0:
                ...
                self.x_t[basin] = {freq: np.array(_x_t) for freq, _x_t in x_t.items()}#####################################
...
@njit()
def validate_samples(x_d: List[np.ndarray], x_s: List[np.ndarray], y: List[np.ndarray], seq_length: List[int], x_t: List[np.ndarray], ####################
    ...
    for i in range(len(frequency_maps)):  # iterate through frequencies
        for j in prange(n_samples):  # iterate through lowest-frequency samples
            ...
            if x_d is not None:
                ...
                _x_t = x_t[i][last_sample_of_freq - seq_length[i] + 1:last_sample_of_freq + 1]##############################
